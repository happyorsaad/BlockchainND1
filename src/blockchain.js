...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTS - messageTS > timeWindow) {
    ...
    getStarsByWalletAddress(address) {
        let self = this;
        return new Promise((resolve, reject) => {
            Promise.allSettled(self.chain.map(b => b.getBData()))
                .then(results => results.filter(r => r.status !== "rejected").map(r => r.value))
                .then(blocks => blocks.filter(b => b.owner === address))
                .then(resolve)
        });
    }
